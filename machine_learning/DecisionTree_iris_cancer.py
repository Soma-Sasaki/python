...
#     for j in range(100):
#         X_iris_train, X_iris_test, y_iris_train, y_iris_test = \
#         train_test_split(X_iris, y_iris, test_size = i, random_state = j)
#         X_cancer_train, X_cancer_test, y_cancer_train, y_cancer_test = \
#         train_test_split(X_cancer, y_cancer, test_size = i, random_state = j)
#
#         iris_tree = DTC()
#         iris_tree.fit(X_iris_train, y_iris_train)
#         cancer_tree = DTC()
...
#         tmp_cancer_test += cancer_tree.score(X_cancer_test, y_cancer_test)
#
#     iris_train_score.append(tmp_iris_train/100)
#     iris_test_score.append(tmp_iris_test/100)
#     cancer_train_score.append(tmp_cancer_train/100)
#     cancer_test_score.append(tmp_cancer_test/100)
#
# fig, ax = plt.subplots()
...
# x = np.arange(1, 11, 1)
#
# #木の最大深度を 1 から 10 まで 1 ずつ変化させる
# for i, test in enumerate(x):
#     iris_tree = DTC(max_depth=x[i], random_state=0)
#     iris_tree.fit(X_iris_train, y_iris_train)
#     cancer_tree = DTC(max_depth=x[i], random_state=0)
#     cancer_tree.fit(X_cancer_train, y_cancer_train)
#
...
#      train_test_split(X_iris, y_iris, test_size=0.3, random_state = 1)
#
#     iris_tree = DTC(max_depth=3, random_state=0)
#     iris_tree.fit(X_iris_train, y_iris_train)
#
#     iris_train_score.append(iris_tree.score(X_iris_train, y_iris_train))
#     iris_test_score.append(iris_tree.score(X_iris_test, y_iris_test))
