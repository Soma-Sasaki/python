...
param_grid = {'max_depth': [2, 4, 8, 16, 32, 64, 128], 'n_estimators': [2, 4, 8, 16, 32, 64, 128]} #グリッドサーチ用のハイパーパラメータ
k = 5 #交差検証の分割数
kfold = KFold(n_splits=k, shuffle=False) #5分割交差検証(シャッフルなし)

#RMSEを基準としてグリッドサーチ
grid_search = GridSearchCV(RFR(), param_grid, cv=kfold, scoring=make_scorer(rmse, greater_is_better=False))
grid_search.fit(X, y)
results = pd.DataFrame(grid_search.cv_results_)
print("Best parameters: {}".format(grid_search.best_params_)) #最も精度が高いパラメータセット表示

rfr_re = RFR(**grid_search.best_params_) #最も精度が高いパラメータセットにおけるモデル作成
...
for i, (ax, (train, test)) in enumerate(zip(axes.ravel(), splitter)):
    ...
    rfr_re.fit(X_train, y_train)
    predict_rfr = rfr_re.predict(X_test)
    MAE.append(mean_absolute_error(y_test, predict_rfr))
    RMSE.append(rmse(y_test, predict_rfr))
    ax.set_title("Split {}".format(i))
    ax.plot(y_test, label="True")
    ax.plot(predict_rfr, label="predicted")
