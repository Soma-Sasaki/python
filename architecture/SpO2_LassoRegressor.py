...
param_grid = {'alpha': [0.01, 0.05, 0.1]} #グリッドサーチ用のハイパーパラメータ
k = 5 #交差検証の分割数
kfold = KFold(n_splits=k, shuffle=False) #5分割交差検証(シャッフルなし)

#RMSEを基準としてグリッドサーチ
grid_search = GridSearchCV(Lasso(), param_grid, cv=kfold, scoring=make_scorer(rmse, greater_is_better=False))
grid_search.fit(X, y)
results = pd.DataFrame(grid_search.cv_results_)
print("Best parameters: {}".format(grid_search.best_params_)) #最も精度が高いパラメータセット表示

lasso_re = Lasso(**grid_search.best_params_) #最も精度が高いパラメータセットにおけるモデル作成
...
for i, (ax, (train, test)) in enumerate(zip(axes.ravel(), splitter)):
    ...
    predict_lasso = lasso_re.predict(X_test)
    MAE.append(mean_squared_error(y_test, predict_lasso))
    RMSE.append(np.sqrt(mean_squared_error(y_test, predict_lasso)))
    ax.set_title("Split {}".format(i))
    ax.plot(y_test, label="True")
    ax.plot(predict_lasso, label="predicted")
